...
// Assuming you have an ApolloProvider wrapping your app higher up
// If this page itself needs to set up Apollo Client, that's a different setup.
...
export default function AdminSignup() { // Renamed component for clarity if used as Admin Signup
  const [form, setForm] = useState({ email: '', password: '', role: 'admin' });
  const [register, { loading, error }] = useMutation(REGISTER, {
    onCompleted: ({ register }) => {
      localStorage.setItem('token', register.token);
      
      window.location.href = '/donaters'; // Redirect after signup
    },
    onError: (err) => {
  ...
  return (
    ...
    <div className="min-h-screen bg-gray-100 py-12 flex items-center justify-center px-4 sm:px-6 lg:px-8">
